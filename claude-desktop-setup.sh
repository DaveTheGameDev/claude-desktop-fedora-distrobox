...
#
#   Install:    ./claude-desktop-setup.sh
#   Share dirs: ./claude-desktop-setup.sh --share ~/Projects --share ~/Notes
#   Update:     ./claude-desktop-setup.sh --update
#   Remove:     ./claude-desktop-setup.sh --remove   [--purge]
#   Help:       ./claude-desktop-setup.sh --help
#
set -euo pipefail
...
# Config / defaults
# ---------------------------------------------------------------------------
VERSION="1.3.0"                # keep in sync with the git tag vX.Y.Z (CI enforces)
REPO_SLUG="DaveTheGameDev/claude-desktop-fedora"
...
Usage:
  claude-desktop-setup.sh [action] [options]
...
rpm_installed() {
  [[ "$SELF" == /usr/bin/claude-desktop-setup ]] && rpm -qf /usr/bin/claude-desktop-setup >/dev/null 2>&1
}
...
build_host_launcher() {
  ...
  log "Writing launcher: $HOST_BIN"
  {
    ...
    printf '# Auto-generated by claude-desktop-setup.sh\n'
