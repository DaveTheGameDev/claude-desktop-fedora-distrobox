...
script_version="$(sed -n 's/^VERSION="\([^"]*\)".*/\1/p' "$root/claude-desktop-setup.sh" | head -n1)"
if [[ "$script_version" != "$version" ]]; then
  echo "error: VERSION in claude-desktop-setup.sh is '$script_version' but building '$version'." >&2
...
cp "$root/claude-desktop-setup.sh" \
...
spec="$topdir/SPECS/claude-desktop-fedora.spec"
if command -v rpmautospec >/dev/null 2>&1; then
  cp "$root/packaging/claude-desktop-fedora.spec" "$spec"
else
  sed "s|^%autochangelog|* $(date +'%a %b %d %Y') Release Bot <noreply@github.com> - $version-1\n- Release $version|" \
    "$root/packaging/claude-desktop-fedora.spec" > "$spec"
fi
